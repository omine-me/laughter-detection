...
#py train.py --config resnet_with_augmentation_finetune --checkpoint_dir ./checkpoints/funetune/resnet_with_augmentation/id --num_workers 4
...
def main():
    ...
    parser.add_argument('--dropout_rate', type=str, default='0.15')
    ...
    def run_epoch(model, mode, device, iterator, checkpoint_dir, optimizer=None, clip=None,
        ...
        def _train_batch(model,device,batch,batch_index=None,clip=None):
            ...
            # F = one_spec
            # window_num = 186*2#43#86#256 # 窓幅のデータ数
            # stride_num = 186#128 # ストライド幅のデータ数  hop = stride
            # power_messy = np.abs(F)
            # phase_messy = np.random.rand(F.shape[0], F.shape[1]) * 2 * np.pi - np.pi
            # F_messy = F.astype(complex)
            # for i in range(F_messy.shape[0]):
            #     for j in range(F_messy.shape[1]):
            #         power = power_messy[i][j]
            #         phase = phase_messy[i][j]
            #         d = complex(power*np.cos(phase), power*np.sin(phase))
            #         F_messy[i][j] = d
            # # _, f_messy = istft(F_messy, fs=sr, nperseg=window_num, noverlap=(window_num-stride_num))
            # f_messy = librosa.istft(F_messy, hop_length=186, win_length=186*2)
            # sd.play(f_messy, sr)
        ...
        if iterator is not None:
            ...
            for i, batch in tqdm(enumerate(iterator)):
                ...
                if log_frequency is not None and (model.global_step + 1) % log_frequency == 0:
                    ...
                    writer.add_scalar('loss/lr', lr, model.global_step)
    ...
    def make_dataframe_from_text_data(data_file_or_lines, h, sr=sample_rate):
        ...
        if type(data_file_or_lines) == type([]):
            ...
            df = pd.read_csv(data_file_or_lines,header=None,names=column_names)
        return df
    ...
    ##################################################################
    val_df = make_dataframe_from_text_data(val_data_text_path, None, sr=sample_rate)
    ...
    while model.global_step < num_train_steps:
        ...
        lines = train_data_text_path
        train_df = make_dataframe_from_text_data(lines, None, sr=sample_rate)
