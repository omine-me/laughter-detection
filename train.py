...
if __name__ == '__main__':
    ...
    sample_rate = 8000
    num_train_steps = 100000#80000#100000
    ...
    if os.path.exists(checkpoint_dir):
        if os.path.exists(checkpoint_dir+'/last.pth.tar'):
            torch_utils.load_checkpoint(checkpoint_dir+'/last.pth.tar', model, optimizer)
        print("From step:", model.global_step)
    else:
    ...
    dataset = Dataset(lang, 7, None, 10 if debug else None, False, False, seed)

    ##################################################################
    ####################  Setup Validation Data  ######################
    ##################################################################
    # val_df = make_dataframe_from_text_data(val_data_text_path, switchboard_val_audios_hash)
    val_df = dataset.encoded_dataset["validation"]

    val_dataset = data_loaders.SwitchBoardLaughterDataset(
        df=val_df,
        audios_hash=None,#switchboard_val_audios_hash,
    ...
    while model.global_step < num_train_steps:
        ...
        train_df = dataset.encoded_dataset["train"]
        ...
        if train_on_noisy_audioset:
            ...
            train_dataset=data_loaders.SwitchBoardLaughterDataset(
                df=train_df,
                audios_hash=None,#switchboard_train_audio_hash,
                feature_fn=augmented_feature_fn,
                batch_size=batch_size,
                sr=sample_rate,
                subsample=True,
